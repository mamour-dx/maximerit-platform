...
async function getResource(type: string, slug: string): Promise<Resource | null> {
  ...
  const res = await payload.find({ collection: "resources", where: { and: [{ type: { equals: type } }, { slug: { equals: slug } }, { status: { equals: "published" } }] }, limit: 1, locale: "en" });
...
export async function generateMetadata({ params }: Args): Promise<Metadata> {
  ...
  if (!r) return {};
  return { title: r.seoTitle || r.title, description: r.seoDescription || r.description || undefined, alternates: { canonical: `/en/ressources/${type}/${slug}/`, languages: hreflang(`/ressources/${type}/${slug}/`, `/en/ressources/${type}/${slug}/`) } };
}

export default async function ResourceDetailEn({ params }: Args) {
  const { type, slug } = await params;
  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-6 py-16">
      <SetLang lang="en" />
      <nav aria-label="Breadcrumb" className="text-sm text-muted"><Link href={`/en/ressources/${type}/`}>{TYPES[type]}</Link> <span aria-hidden>›</span> <span>{r.title}</span></nav>
      ...
      {r.gated ? (
        <section className="mt-10 rounded-[var(--radius)] border border-border p-6">
          <h2 className="text-2xl font-bold">Get this content</h2>
          <p className="mt-1 mb-5 text-sm text-muted">Access “{r.title}” — we&apos;ll send it to you by email.</p>
          <LeadForm source={`resource:${slug}`} lang="en" ctaLabel="Get the content" />
        </section>
      ) : (
