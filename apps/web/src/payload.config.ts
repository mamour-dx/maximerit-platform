...
export default buildConfig({
  ...
  collections: [
    ...
    {
      slug: "jobs",
      admin: { useAsTitle: "title", group: "Recrutement" },
      access: { read: () => true, create: isAuthenticated, update: isAuthenticated, delete: isAdmin },
      fields: [
        { name: "title", type: "text", required: true, localized: true },
        { name: "slug", type: "text", required: true, unique: true, index: true },
        ...
        { name: "experienceMin", type: "number" },
        {
          name: "contractType",
          type: "select",
          defaultValue: "cdi",
          options: ["cdi", "cdd", "mission-interim", "consultant-freelance", "fifo-roster", "stage"].map((v) => ({ label: v, value: v })),
        },
        {
          name: "status",
          type: "select",
          ...
          options: [
            ...
            { label: "Pourvue", value: "pourvue" },
            { label: "Archivée", value: "archivee" },
          ],
        },
        { name: "publishedAt", type: "date" },
        { name: "expiresAt", type: "date" },
        { name: "similarJobs", type: "relationship", relationTo: "jobs", hasMany: true },
      ],
    },
    {
      slug: "applications",
      admin: { useAsTitle: "id", group: "Recrutement" },
      // Créées via /apply (overrideAccess) ; jamais lisibles publiquement (données candidats).
      access: { read: isAuthenticated, create: isAuthenticated, update: isAuthenticated, delete: isAdmin },
      fields: [
        { name: "job", type: "relationship", relationTo: "jobs" },
        { name: "candidate", type: "relationship", relationTo: "candidates", required: true },
        { name: "cv", type: "relationship", relationTo: "cvs" },
        {
          name: "status",
          type: "select",
